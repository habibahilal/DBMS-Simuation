...
public class DBApp {
	...
	public static DBApp db = new DBApp();
...
//			System.out.println(resultEntry.size());
			...
			for (int i = 0; i < resultEntry.size(); i++) {
				...
				for (int j = 0; j < entryPage.records.size(); j++) {
					...
					if (compare(currRecord.get(colx),currEntry.colsValues.getX()) == 0&&compare(currRecord.get(coly), currEntry.colsValues.getY()) == 0&&
							compare(currRecord.get(colz), currEntry.colsValues.getZ()) == 0) {
